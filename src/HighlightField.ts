...
export class HighlightFields {
	private _currentDecoration: vscode.TextEditorDecorationType | null;
	...
	public ShowHighlights(itemLocation: string | null, hl7Schema: HashTable<SegmentSchema>, backgroundColor: string) {
		...
		if (fieldSelectionList.length > 0) {
			if (this._currentDecoration !== null) {
		...
		fieldSelectionList = this.AddDecorations(activeEditor, locationHashtable, delimiters.Field)
		...
		this._currentDecoration = highlightDecorationType;
	...
	private AddDecorations(activeEditor: vscode.TextEditor, locationHashtable: HashTable<number[]>, fieldDelimiter: string): any[] {
